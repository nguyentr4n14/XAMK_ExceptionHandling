...
namespace ExceptionHandling
{
    internal class Program
    {
        ...
        // Function for exception propagation
        static void CheckNumber(int number)
        {
            if (number > 100)
            {
                throw new ArgumentOutOfRangeException("Error: Number must be 100 or less!");
            }
        }
        ...
        // Function with logging for exception
        static void CheckNumberWithLogging(int number)
        {
            if (number > 100)
            {
                string errorMessage = $"Error: Number {number} is greater than 100.";
                Console.WriteLine($"Logging: {errorMessage}");
